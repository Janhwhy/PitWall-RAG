...
circuit_agent.py
----------------
Defines the CircuitAgent class, a specialized BaseAgent focused on circuit characteristics
(specifically Monaco), analyzing track-specific incidents (Safety Cars, Virtual Safety Cars,
yellow flags), overtaking difficulty, and predicting position change likelihood.
"""
...
class CircuitAgent(BaseAgent):
    """
    Agent specializing in Monaco circuit analysis, overtaking difficulty, and safety car impact.
    """

    def __init__(self) -> None:
        """Initialise CircuitAgent with name 'circuit_agent' and access to radio and laps."""
        super().__init__(name="circuit_agent", collections=["radio", "laps"])

    def run(self, query: str, race: str | None = None, year: int | None = None) -> dict[str, Any]:
        """
        Analyse Monaco track incidents (Safety Cars, yellow flags, overtaking difficulties) to answer queries.
        ...
        system_prompt = (
            "You are a Formula 1 circuit specialist, specifically an expert on the Monaco Grand Prix. "
            "You analyze track incidents (Safety Cars, Virtual Safety Cars, track blocks), "
            "overtaking difficulties, track evolution, and the likelihood of track position changes.\n\n"
            "RULES:\n"
        ...
        user_prompt = (
            ...
            "## Question\n"
            f"{query}\n\n"
            "Please provide a precise strategy answer focused on circuit conditions, overtaking, "
            "Safety Car probabilities, and position changes based strictly on the retrieved context."
        )
